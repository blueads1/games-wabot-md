...
handler.before = async function (m) {
    ...
    if (!(id in this.math)) return this.sendButton(m.chat, 'Soal itu telah berakhir', null, [['math', '/math']], m)
    if (m.quoted.id == this.math[id][0].id) {
        ...
        if (m.text == math.result) {
            ...
            delete this.math[id]
            this.sendButton(m.chat, `*Jawaban Benar!*\n+${math.bonus} XP`, null, [['again', `/math ${math.mode}`]], m)
        } else {
            if (--this.math[id][2] == 0) {
                clearTimeout(this.math[id][3])
                delete this.math[id]
                this.sendButton(m.chat, `*Kesempatan habis!*\nJawaban: *${math.result}*`, null, [['again', `/math ${math.mode}`]], m)
